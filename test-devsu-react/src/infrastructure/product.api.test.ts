...
function mockSuccess<T>(data: T, status = 200) {
  return jest.spyOn(httpClient, "request").mockResolvedValue({
    data,
    status,
    statusText: "OK",
    headers: {},
...
function mockAxiosError(status: number, data?: unknown) {
  ...
  return jest.spyOn(httpClient, "request").mockRejectedValue(error);
}

describe("repositorio API de produtos", () => {
  ...
  it("existsById llama al endpoint de verificasion", async () => {
